...
export function initNotes(){
  ...
  const getTabs = ()=>{
    const tabs = Array.isArray(window.__notesTabList) && window.__notesTabList.length ? window.__notesTabList : [{ id:'memo', name:'메모', order:0 }];
  ...
  const setActive = async (tabId)=>{
    ...
    if(window.cloudSaveNotesNow) window.cloudSaveNotesNow(prev.tabId, prev.value).catch(e=>console.error(e));
    else if(window.cloudSaveNotesFor) Promise.resolve(window.cloudSaveNotesFor(prev.tabId, prev.value)).catch(e=>console.error(e));

    window.__notesActiveTabId = tabId;
    if(window.cloudSetActiveNotesTab) Promise.resolve(window.cloudSetActiveNotesTab(tabId)).catch(e=>console.error(e));
    render();
  ...
  const backupTab = (tabId)=>{
    ...
    const safeName = String(tab?.name || 'memo').replace(/[\\/:*?"<>|#%{}~&]/g,'_').trim() || 'memo';
    const lines = [
      `메모 탭 백업`,
      `탭: ${tab?.name || '메모'}`,
    ...
    downloadTextFile(`${safeName}_${now.getFullYear()}${pad(now.getMonth()+1)}${pad(now.getDate())}_${pad(now.getHours())}${pad(now.getMinutes())}.txt`, lines.join('\n'));
  ...
  const downloadAllNotes = ()=>{
    ...
    const lines = ['메모 전체 백업', `다운로드 시각: ${dateText}`, '', '========================================', ''];
    ...
    downloadTextFile(`memo_backup_${now.getFullYear()}${pad(now.getMonth()+1)}${pad(now.getDate())}_${pad(now.getHours())}${pad(now.getMinutes())}.txt`, lines.join('\n'));
  ...
  function openSettings(tabId){
    ...
    openTabSettings({
      ...
      tab,
      getTabs,
      onSave:async(id,name)=>{ await window.cloudRenameNotesTab?.(id,name); },
      onDelete:async(id)=>{ await window.cloudDeleteNotesTab?.(id); },
      onBackup:async(id)=>backupTab(id),
      onReorder:async(next)=>{ await window.cloudReorderNotesTabs?.(next); },
      onCreate:null
    });
  }
